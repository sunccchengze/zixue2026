...
('13','<b>1. 容斥公式是什么</b><br/>求“至少有一个事件发生”的概率时，先把各事件概率相加；这样两个事件共同发生的部分被重复计算，所以减去所有两两交集；三个事件共同发生的部分又被减多了，所以加回所有三重交集。以后按“加、减、加、减……”交替进行。这就是容斥原理。<br/>例如三个事件时：<br/>P(A<sub>1</sub>∪A<sub>2</sub>∪A<sub>3</sub>)=P(A<sub>1</sub>)+P(A<sub>2</sub>)+P(A<sub>3</sub>)-P(A<sub>1</sub>∩A<sub>2</sub>)-P(A<sub>1</sub>∩A<sub>3</sub>)-P(A<sub>2</sub>∩A<sub>3</sub>)+P(A<sub>1</sub>∩A<sub>2</sub>∩A<sub>3</sub>)。<br/>一般地：并集概率=所有单个事件概率之和-所有两重交集概率之和+所有三重交集概率之和-…+(-1)<super>n+1</super>乘以n重交集的概率。<br/><b>2. 为什么它正确</b><br/>若某个试验结果恰好同时属于r个事件，那么右边对它的净计数为C(r,1)-C(r,2)+C(r,3)-…+(-1)<super>r+1</super>C(r,r)=1，所以并集中的每个结果最终恰好被计算一次。<br/><b>3. 用数学归纳法证明</b><br/>① 基础步骤：n=2时，公式为P(A<sub>1</sub>∪A<sub>2</sub>)=P(A<sub>1</sub>)+P(A<sub>2</sub>)-P(A<sub>1</sub>∩A<sub>2</sub>)，即两事件加法公式，成立。<br/>② 归纳假设：假设任意n个事件的容斥公式成立，记U<sub>n</sub>=A<sub>1</sub>∪…∪A<sub>n</sub>。<br/>③ 从n推到n+1：由两事件加法公式，<br/>P(U<sub>n</sub>∪A<sub>n+1</sub>)=P(U<sub>n</sub>)+P(A<sub>n+1</sub>)-P(U<sub>n</sub>∩A<sub>n+1</sub>)。<br/>又有U<sub>n</sub>∩A<sub>n+1</sub>=(A<sub>1</sub>∩A<sub>n+1</sub>)∪…∪(A<sub>n</sub>∩A<sub>n+1</sub>)。这仍是n个事件的并，因此可使用归纳假设展开。P(U<sub>n</sub>)给出所有不含A<sub>n+1</sub>的交集项；减去P(U<sub>n</sub>∩A<sub>n+1</sub>)后，恰好补上所有含A<sub>n+1</sub>的交集项，而且符号仍按交集阶数正负交替。两部分合在一起，正是n+1个事件的容斥公式。<br/>所以由数学归纳法，容斥公式对任意正整数n成立。'),
