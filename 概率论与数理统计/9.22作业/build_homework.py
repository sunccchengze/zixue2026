...
S=[
('1','(1) {白,黑}。 (2) {白白,白黑,黑白,黑黑}。 (3) {0,1,2}。 (4) {1,2,3,4,5}。 (5) {10,11,12,…}。 (6) {次次, 正次次, 正正正正, 正正正次, 正正次正, 正正次次, 正次正正, 正次正次, 次正正正, 次正正次, 次正次正, 次正次次}；即首次出现“次次”便停止，或查满4件且前三次中未出现“次次”。 (7) Ω={(x,y):x²+y²≤R²}。'),
('3','直接按“交=且、横线=不”翻译：(1) 是男生、爱好数学且不是班干部；(2) 不是男生、爱好数学且不是班干部；(3) 由德摩根律，既不是男生也不是班干部；(4) 是男生，但既不爱好数学也不是班干部。'),
('5','(1) (A∩B)∪(A−B)=A，故再并A<super>c</super>得Ω。 (2) 令D=(A−B)∪(B−A)（A、B恰有一个发生）。在A∪B内，D<super>c</super>表示A、B同时发生，所以(A∪B)∩D<super>c</super>=A∩B；再减去B，得∅。'),
('7','1000=10³，故每条棱被分成10段。恰有两面涂红的是12条棱上除去角块的小块，共12(10−2)=96个。因此P=96/1000=12/125。'),
('9','设两船到达时刻为x、y。直接计算“发生等待”的两条对角带：当x≤y时，需x＜y＜x+1，面积为23×1+1×1/2=47/2；当y＜x时，需y＜x＜y+2，面积为22×2+2×2/2=46。两区域不相交，故总面积为47/2+46=139/2。因此P(等待)=(139/2)/24²=139/1152。注意第一条带末端是半个1×1三角形，不能按面积24计算。'),
('11','因A∩C=∅，故A∩B∩C=∅，(1) 0。容斥：(2) 3/4−1/8−1/8−0+0=1/2。(3) 1−1/2=1/2。'),
