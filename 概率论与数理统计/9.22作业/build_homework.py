...
('3','从某班学生中任选一名，A={男生}，B={数学爱好者}，C={班干部}。分别说明下列事件的含义：<br/>(1) AB C̄；　(2) ĀB C̄；　(3) A∪C 的对立事件；　(4) A−(B∪C)。'),
('5','化简：<br/>(1) AB∪(A−B)∪Ā；<br/>(2) (A∪B)·[(A−B)∪(B−A)]的对立事件−B。'),
('7','一块表面都涂成红色的正方体被锯成1000个同样大小的小正方体。将小正方体混匀后任取一个，求其两面涂有红色的概率。'),
('9','两艘船停靠同一码头，到达时刻在一昼夜内独立且均匀；停靠时间分别为1 h和2 h。求至少一艘船需要等待一段时间的概率。'),
('11','设P(A)=P(B)=P(C)=1/4，P(AB)=P(BC)=1/8，P(AC)=0。求：(1) A、B、C都发生；(2) 至少一个发生；(3) 都不发生的概率。'),
('13','设A₁,…,Aₙ为任意n个事件，用数学归纳法证明容斥公式。'),
...
('33','证明：若A、B、C相互独立，则A∪B、AB及A−B都与C相互独立。'),
('35','8张同形卡片：1,2,3,4涂红；1,2,3,5涂白；1,6,7,8涂黑。任取一张，A、B、C分别表示有红、白、黑色。验证P(ABC)=P(A)P(B)P(C)，但A、B、C不两两独立。'),
...
S=[
('1','(1) {白,黑}。 (2) {白白,白黑,黑白,黑黑}。 (3) {0,1,2}。 (4) {1,2,3,4,5}。 (5) {10,11,12,…}。 (6) {次次, 正次次, 次正次次, 正正正正, 正正正次, 正正次正, 正次正正, 正次正次, 次正正正, 次正正次, 次正次正}；即首次出现“次次”便停止，或查满4件且此前未出现“次次”。 (7) Ω={(x,y):x²+y²≤R²}。'),
('3','直接按“交=且、横线=不”翻译：(1) 是男生、爱好数学且不是班干部；(2) 不是男生、爱好数学且不是班干部；(3) 由德摩根律，既不是男生也不是班干部；(4) 是男生，但既不爱好数学也不是班干部。'),
('5','(1) AB∪(A−B)=A，故再并Ā得Ω。 (2) 令D=(A−B)∪(B−A)（A、B恰有一个发生）。在A∪B内，D̄表示A、B同时发生，所以(A∪B)D̄=AB；再减B得∅。'),
('7','1000=10³，故每条棱被分成10段。恰有两面涂红的是12条棱上除去角块的小块，共12(10−2)=96个。因此P=96/1000=12/125。'),
('9','设两船到达时刻(x,y)均匀分布于24×24正方形（按一昼夜首尾不相接的通常模型）。不等待区域满足y≥x+1或x≥y+2，面积为(23²+22²)/2。因此P(等待)=[24²−(23²+22²)/2]/24²=139/1152。'),
('11','因AC=∅，故ABC=∅，(1) 0。容斥：(2) 3/4−1/8−1/8−0+0=1/2。(3) 1−1/2=1/2。'),
('13','n=2时公式即加法公式。假设对n成立，令U=∪ᵢ₌₁ⁿAᵢ，则P(U∪Aₙ₊₁)=P(U)+P(Aₙ₊₁)−P(UAₙ₊₁)。对P(U)用归纳假设；又UAₙ₊₁=∪ᵢ₌₁ⁿ(AᵢAₙ₊₁)，再对这n个事件使用归纳假设。按交集阶数合并，恰得到n+1个事件的容斥公式，证毕。'),
...
('19','总数C(52,4)。(1) 4C(13,4)/C(52,4)。(2) 每花色各1张：13⁴/C(52,4)。(3) 为(2)的补：1−13⁴/C(52,4)。(4) 1−C(48,4)/C(52,4)。'),
('21','把3个可区分球独立放入4盒，共4³种。(最大=1)：4·3·2=24，P=3/8。(最大=3)：4种，P=1/16。(最大=2)：余下64−24−4=36，P=9/16。'),
('23','总数C(30,10)。若恰有k双：先选k双，再从余下15−k双中选10−2k双，并从每双取1只，数为C(15,k)C(15−k,10−2k)2^(10−2k)。(1) k=2，除以C(30,10)。(2) 对k=2,3,4,5求和后除以C(30,10)。'),
('25','P(AB)=P(A)P(B|A)=1/12。由P(A|B)=1/2得P(B)=P(AB)/(1/2)=1/6。故P(A∪B)=1/4+1/6−1/12=1/3。'),
...
('35','A={1,2,3,4}，B={1,2,3,5}，C={1,6,7,8}，各概率均1/2；ABC={1}，P(ABC)=1/8=(1/2)³。但AB={1,2,3}，P(AB)=3/8≠1/4=P(A)P(B)，故至少A、B不独立，三事件不两两独立。'),
('37','用补事件：P(至少一人命中)=1−(1−4/5)(1−2/3)(1−3/4)=1−1/60=59/60。'),
('39','令x=P(A), y=P(B)。独立性给x(1−y)=1/4，y(1−x)=1/4。两式相减得x=y。于是x(1−x)=1/4，即(x−1/2)²=0，故P(A)=P(B)=1/2。')]
story=[P('9.22 概率论作业—完整解析',h1),P('孙承泽　2253710052　能动强基2501',small),P('说明：组合数记为C(n,k)；AB表示A∩B。',small)]
...
report += [t,P('三、优点',h2),P('1. 覆盖链条完整：从集合语言到概率模型，再到条件概率和独立性。<br/>2. 计算题与证明题并存，既检查操作熟练度，也检查逻辑表达。<br/>3. 15、17、19、21、23题形成组合计数梯度；29、31题形成全概率—贝叶斯衔接；33、35、39题能纠正常见的独立性误区。',sol),P('四、主要风险与易错点',h2),P('第1题须区分“试验结果”与数值记录；第3、5题补集横线容易误读；第9题需明确24小时边界模型；第11题要利用AC=∅推出ABC=∅；第15、19、23题易重复计数；第31题后验概率分母须用全概率；第35题展示“三者乘积等式成立”不等于两两独立；第39题须把“仅A/仅B”翻译为A B̄和Ā B。',sol),P('五、工作量与建议',h2),P('预计熟练学生完成题面约150—210分钟，完整规范书写约210—300分钟。四面答题纸只能提供“关键过程”空间，复杂证明和组合题建议另附草稿纸。建议评分时将题3、5的集合符号识读与数学推理分开；第9题若学生说明采用环形24小时模型，应按其模型复核，而不应仅按单一数值判错。',sol),P('六、质量评级',h2),P('知识覆盖：A；难度梯度：A−；表述清晰度：B+（扫描补集横线与第9题边界存在歧义）；计算训练价值：A；概念诊断价值：A。综合评价：A−。',sol)]
