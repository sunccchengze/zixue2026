...
('3','从某班学生中任选一名，A={男生}，B={数学爱好者}，C={班干部}。分别说明下列事件的含义：<br/>(1) ĀBC̄；　(2) (ABC)的对立事件；　(3) A∪C 的对立事件；　(4) A−(B∪C)。'),
('5','化简：<br/>(1) AB∪(A−B)∪Ā；<br/>(2) (A∪B)·(A−B的对立事件)∪(B−A的对立事件)−B。'),
('7','一袋装有都涂成红色的1000个同样大小的小正方体。将它们均匀混合，任取一个，求该正方体两个面涂有红色的概率。'),
...
S=[
('1','(1) {白,黑}。 (2) {白白,白黑,黑白,黑黑}。 (3) {0,1,2}。 (4) {1,2,3,4,5}。 (5) {10,11,12,…}。 (6) {次次, 正次次, 次正次, 正正次次, 正次正次, 次正正次, 正正正正, 正正正次, 正正次正, 正次正正, 次正正正}；等价地为长度≤4且首次出现“次次”即停止，或长度4而此前无“次次”的序列。 (7) Ω={(x,y):x²+y²≤R²}。'),
('3','(1) 是男生、爱好数学且不是班干部；(2) A、B、C不全同时发生；(3) 既不是男生也不是班干部；(4) 是男生，但既不爱好数学也不是班干部。'),
('5','(1) AB∪A B̄=A，再与Ā并得Ω。 (2) 逐项用集合恒等式与德摩根律化简；注意原扫描件该式横线层级较密，提交前应按教材原式逐横线核对，避免把“事件的补”误读为仅字母的补。'),
('7','每个小正方体原先所有6面均涂红，故事件必然发生，P=1。（若题意原指“大正方体切成1000块”，则需给出切割方式；原题文字明确为1000个小正方体分别都涂红。）'),
