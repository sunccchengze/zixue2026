...
├── 白皮书/                         ★ 教材线产物：十二讲图文并茂伴读白皮书 + 装配器 + 六项机器验收 + 可运行代码（入口 `白皮书/README.md`）
└── 论文01-ML气动外形优化/
...
- **产物**：`白皮书/机器学习与气动外形优化自学白皮书.md`（自动装配，114k 字符，**不许手改**）；
- **结构**：前言 + 第零章 + 四篇 10 讲 + 第五篇代码 + 第六篇批判 + 讲 12 整合；`lec-NN` 与会话一一映射；
...
- **当前状态**：11 / 12 讲完成（讲 08 待重建），六项验收全绿。欠账表在 `白皮书/README.md` §五。
