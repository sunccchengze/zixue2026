...
# 每个代码模块的三件套：① 干什么（一句）② 跑完应看到的关键数字 ③ 改一个常数重跑的小实验
CODE_NOTES = {
    "curse.py": (
        "把「维度灾难」从口号变成算术：给你每维点数、所需样本量，以及高维里「点都在边缘」的实证。",
        "样本数与边界距离同时坍缩、邻居距离几乎不变，末尾 `selftest: PASS`。",
        "把 d 从 20 改成 8，先预测每维点数会变成多少，再跑，对答案。",
    ),
    "ego.py": (
        "EI 加点准则 + 20 步序贯优化：在 y = x·sin(x) 上演示「下一个点该算在哪」。",
        "找到的最优点与真值差 0.0050 量级、EI 僵住步数 0/12，末尾 `selftest: PASS`。",
        "把初始样本从 3 个加到 8 个，先猜 EI 僵住步数怎么变，再跑。",
    ),
    "gek.py": (
        "间接梯度增强 Kriging：用一阶泰勒造虚拟点把导数喂进去，看它值不值这份算力。",
        "多随机设计平均只有 1.13× 的改善、n=10 时反而 0.93×，末尾 `selftest: PASS`。",
        "把样本数从 10 加到 40，先猜比值怎么变，再跑。",
    ),
    "kriging.py": (
        "最小可用 Kriging：高斯相关 + 常数趋势 + θ 的极大似然网格搜索，输出 μ 与 σ。",
        "训练点插值误差 < 1e-2、域外 σ 与域内 σ 之比 25.29，末尾 `selftest: PASS`。",
        "把 θ 手工放大 10 倍，先猜域外 σ 的比值怎么变，再跑。",
    ),
    "modal.py": (
        "20 维玩具翼型：PCA 模态收缩 + 几何有效性门，实测「每维点数」与「白烧 CFD 概率」。",
        "r=6 @99% 时每维点数 3.16、一次 CFD 白烧概率从 93.5% 降到 2.8%，末尾 `selftest: PASS`。",
        "把能量线从 99% 放宽到 95%，先猜保留维数与每维点数怎么变，再跑。",
    ),
    "pca.py": (
        "PCA 三步（中心化 → 协方差 → 特征分解），打印特征值与累计方差保留曲线。",
        "前 1 维重构 RMSE = 0.00324（与噪声 σ = 0.05 同量级）、k=1 保留 99.12%，末尾 `selftest: PASS`。",
        "把噪声 σ 从 0.05 改到 0.20，先猜重构 RMSE 是否还同量级，再跑。",
    ),
}
...
def build() -> str:
    ...
    if os.path.isdir(CODE_DIR):
        for fn in sorted(os.listdir(CODE_DIR)):
            ...
            out.append(f"### `{fn}`")
            out.append("")
            note = CODE_NOTES.get(fn)
            if note:
                out.append(f"**① 这代码干什么**：{note[0]}")
                out.append("")
                out.append(f"**② 跑完你应看到**：{note[1]}")
                out.append("")
                out.append(f"**③ 你的任务**：{note[2]}")
                out.append("")
            out.append("```python")
