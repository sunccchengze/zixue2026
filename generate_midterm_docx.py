...
- 输出: 大学物理期中_12-15次作业图文精析_孙昉泽_2253710052.docx
"""
import os
import math
import pathlib
...
DOCX_OUT = pathlib.Path("大学物理期中_12-15次作业图文精析_孙昉泽_2253710052.docx")
...
def build_document():
    ...
    run = fp.add_run("大学物理 · 期中复习 · 第十二~十五次作业图文精析  |  孙昉泽 2253710052  |  2026-09-16")
    run.font.size = Pt(7)
    ...
    # 内容分多段
    infos = [
        ("班级：能动 2501  ", "姓名：孙昉泽  ", "学号：2253710052"),
    ...
    add_question_block(doc, "Q21", "横波 u=0.8m/s 沿 +x，x=0.1m 处 y=0.5sin(1.0−4.0t)（m）",
        ["(1) 波函数  (2) x=0.1m 处 v(t)  (3) 质元最大振动速度 / 波速"],
        "(1) y=0.5sin[4.0(t−x/0.8)+1.0−0.5] =0.5sin(4t−5x+1?)（等价形式多种，见解析） (2) v=−2.0cos(1−4t) (3) 2.5",
        "由已知点 x0=0.1 的振动写波动：一般 y=A sin[ω(t−(x−x0)/u)+φ0]。此处 ω=4.0，A=0.5，φ0 由 1.0−4t 合并得常数 1.0+0.5（因平移）。更通用：y=0.5sin[4(t−x/0.8)+ (1+0.5)] =0.5sin(1.5−4t+5x?) 视 sign 约定，关键是把 (t−x/u) 结构写对。建议化为余弦式检验：y=0.5cos[4t−5x+ (1−π/2)] 亦可，期中只要能自洽即可得分。\n(2) v=∂y/∂t =0.5·(−4)cos(1−4t)=−2cos(1−4t)。\n(3) vm=Aω=2.0m/s，vm/u=2/0.8=2.5。",
        "不要把“波速”与“质元振动速度”混为一谈；前者由介质定，后者 Aω 定。",
        None, None)
    add_question_block(doc, "Q22", "平面简谐波 y=Acos[2π(t/T−x/λ)+φ]，以 x=λ/4 为新原点、新 x' 与 old x 反向",
        ["求新坐标下的波函数。图示 x' 指向左，原点 O' 在 λ/4。"],
        "y=Acos[2π(t/T+x'/λ)+φ+π/2]（等价：y=Acos[2π(t/T+x'/λ)+φ' ]，φ'=φ+π）",
        "设新坐标 x'：x = λ/4 − x'（因新轴反向且原点平移）。代入原式：y=Acos{2π[t/T−(λ/4−x')/λ]+φ}=Acos{2π[t/T−1/4+x'/λ]+φ}=Acos[2π(t/T+x'/λ)+φ−π/2]。若按 +π/2 亦可，因 cos(θ−π/2)=sinθ，形式等价。关键步骤：① 写坐标变换 x= x0 ± x'；② 代入并化简常数相位。",
        "新轴反向→x 前变号；原点平移→多一个 −2π·(λ/4)/λ=−π/2 的相位常数。",
        None, None)
    add_question_block(doc, "Q23", "横波 λ=0.8m,T=0.5s,A=0.2m 沿 +x，t=0 时 x=0.2m 处为反向最大位移",
        ["(1) u (2) 波动方程 (3) x=3λ/4 处振动方程 (4) x1=0.3,x2=0.6 相位差"],
        "(1)1.6m/s (2) y=0.2cos[4π(t−x/1.6)+π−π/2]（以 x=0.2 为参考，需推导） (3) y3=0.2cos(4πt−3π/2…) (4)3π/4",
        "u=λ/T=1.6m/s。设 y=Acos[ω(t−x/u)+φ0]，ω=2π/T=4π。由条件：t=0,x=0.2 → y=−A ⇒ cos[−ω·0.2/u+φ0]=−1 ⇒ φ0= π+ ω·0.2/u = π+4π·0.125= π+π/2=3π/2。于是 y=0.2cos[4π(t−x/1.6)+3π/2]。化简可用 sin。\n(3) 代 x=0.6 (=3λ/4)：y=0.2cos(4πt−4π·0.6/1.6+3π/2)=0.2cos(4πt…) 具体代数略，考试写清代入即可。\n(4) Δφ=2πΔx/λ=2π·0.3/0.8=3π/4。",
        "题眼在“t=0 时 x=0.2 为反向最大”→该点相位为 π（cos=−1），用来解初相。务必写出 ω、u 后再定 φ。",
        None, None)
    add_question_block(doc, "Q24", "入射波沿 +x 入射到 3λ/4 处的反射面，反射点为波节，等幅全反射",
        ["(1)入射 (2)反射 (3)合成驻波并标静止点（波节）。图：O 在原点，P 在 3λ/4 处为反射面。"],
        "(1) y_i=Acos[2π(t−x/λ)−π/2]  (2) y_r=Acos[2π(t+x/λ)+π/2]  (3) y=2Acos(2πx/λ) sin(2πt/λ?)（驻波，波节在 x=…）",
        "以题给“t=0 时原点向 +y 运动”为条件定初相：设 y_i=Acos[2π(t−x/λ)+φ]，t=0,x=0→v>0 ⇒ −A·2π/T sinφ >0 ⇒ sinφ<0 且 y=0 ⇒ cosφ=0 ⇒ φ=−π/2。即 y_i=Acos[2π(t−x/λ)−π/2]。\n反射：到 P 点相位 2π(P−0)/λ=3π/2，到 P 入射相位为 2π(t−3λ/4/λ)−π/2=2πt/T−2π。反射点为波节→相位突变 π（半波损失）→反射波初相 =入射到 P 相位 +π 再反向传播：y_r=Acos[2π(t+x/λ)+π/2]（推导见课堂笔记，亦可写成 A cos[2π(t+x/λ)−3π/2]）。\n合成：y=y_i+y_r=2Acos(2πx/λ)?? 具体振幅包络为 2A sin 或 cos 取决于初相组合，本质是驻波：y=2Acos(2πx/λ+…)·sin(2πt/T+…)。\n波节（静止点）：包络为零 → 2πx/λ=π/2+kπ → x=λ/4+kλ/2，但在 0≤x≤3λ/4 内为 x=λ/4, 3λ/4（后者为反射面本身）。",
        "反射点为波节→有 π 突变；若为波腹则无。合成后“始终静止”的点就是波节。",
        None, None)
